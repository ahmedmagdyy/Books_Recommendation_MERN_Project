...
class FormAddBook extends React.Component {
  ...
  addSubmitForm = event => {
    ...
    const catItem = this.props.categories.filter(
      item => item.name === this.state.categorySelect,
    )[0];
    const authItem = this.props.authors.filter(
      item =>
        item.first_name + " " + item.last_name === this.state.authorSelect,
    )[0];
    ...
    fetch("http://localhost:5000/books", {
      ...
      .then(data => {
        console.log(data);
        this.props.addItemList(data, "Book");
      })
  ...
  componentWillMount() {
    ...
    const authorArr = this.props.authors.map(
      ...
      >
        ...
        <FormGroup>
          ...
          <Input
            type="select"
            name="select"
            onChange={this.handeCategoryChange}
        ...
        <FormGroup>
          ...
          <Input
            type="select"
            name="select"
            onChange={this.handeAuthorChange}
